...
public class CaNhanDAO {
    public static nhanvien getThongTinCaNhanDAO(int MaNhanVien){
        nhanvien objNV = new nhanvien();
        String sql = "SELECT * FROM sachgiare.nhanvien Where MaNhanVien = ?";
    ...
    public static List<chamcong> getChamCongDAO(int MaNhanVien){
        List<chamcong> lstCC = new ArrayList<>();
        String sql = "SELECT * FROM sachgiare.chamcong Where MaNhanVien = ? ORDER BY MaChamCong DESC";
        Connection conn = ConnectDB.getConnect();
        PreparedStatement sttm;
        try {
            sttm = conn.prepareStatement(sql);
    ...
    public static int insertChamCongDAO(int MaNhanVien){
        String sql = "INSERT INTO sachgiare.chamcong(MaNhanVien) VALUES (?)";
        Connection conn = ConnectDB.getConnect();
        PreparedStatement sttm;
        int result;
        try {
    ...
    public static chamcong getLastChamCongDAO(int MaNhanVien){
        chamcong objCC = new chamcong();
        String sql = "SELECT * FROM chamcong WHERE MaNhanVien = ? ORDER BY MaChamCong DESC LIMIT 1";
        Connection conn = ConnectDB.getConnect();
        PreparedStatement sttm;
        try {
            sttm = conn.prepareStatement(sql);
    ...
    public static int updateThongTinCaNhanDAO(nhanvien objNV){
        String sql = "UPDATE nhanvien SET HoTen=?,GioiTinh=?,NgaySinh=?,QueQuan=?,SoDienThoai=?,MatKhau=? WHERE MaNhanVien = ?";
        Connection conn = ConnectDB.getConnect();
        PreparedStatement sttm;
        int result;
        try {
            ...
            sttm.setInt(7, objNV.getMaNhanVien());
            result = sttm.executeUpdate();
            if(result == 1){
                return 1;
            }else{
